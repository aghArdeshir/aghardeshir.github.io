...
Also, try to describe the MR in 1 short line. If you can't, then it probably means something should be separated into another task/MR. The same is true about commit messages and commits!

## Resolving comments on the MRs

Do not resolve the comments in the MR if you are not the author of the comment. Leave it for the author of the comment to resolve it. If you think you have resolved the comment, the best thing to do is to leave a note: "Thanks, Done!" preferably with a link to the commit that solves the requested change. This way the reviewer gets a chance to double-check the comment, make sure you understood the comment correctly, review the new change, and if all was good, they can resolve the comment, otherwise, they can continue requesting changes or asking questions in the thread.

The big downside of resolving the comments if you are not the author of the comment, is that if you resolve the comments, the author must come back, expand all resolved comments (because most UIs collapse the resolved comments), and make sure you've done them in the right way. If they find one comment that was mistakenly resolved or the comment author wants to add more notes to it, they will unresolve the comment to continue on the thread. They do and then you come again. Reading all comments, Fixing or replying to them, then resolving them all again. If the comment author comes back, they don't know where they were. Because the UIs collapse resolved comments and say "Resolved by Someone". They have to go through the process of opening all the resolved comments again to find out where there is an unfinished conversation.

Resolving comments is the right of the comment author (usually the reviewer) to be able to track their review. So if next time they see a "Resolved by Someone" comment and that Someone is the author of the comment themselves, they know it's all good then and can pass over that without expanding it. Then one time they resolve a comment, notice all comments are resolved, and if all comments are resolved and all files are viewed, means the MR can be approved. ✅

The term "resolved" means that both the author of the merge request and the author of the comment have come to a conclusion and agree to the current state of the code 👍 well, the author of the merge request always agrees to the current state of the code, if they were not, they would have done it in a better way. So only if the comment author also agrees to the outcome of the conversation, then and only then does it mean the comment/thread is "resolved"! ✅

One trick I use when someone else has resolved my comment, is I unresolve it first, check it, and if all was good, I resolve it again, just so that the next time it says that this comment is resolved by me.
